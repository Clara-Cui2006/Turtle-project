...
export function App() {
  ...
  const refreshLists = useCallback(async () => {
    ...
    if (selected) {
      ...
      setDetail(value); setCourseId(value.courseId);
    }
  ...
  async function newCourse() { const name = prompt('课程名称'); if (!name?.trim()) return; const teacher = prompt('授课教师（可留空）') ?? ''; const description=prompt('课程描述（可留空）')??'';const tags=(prompt('标签（用逗号分隔，可留空）')??'').split(/[,，]/).map((item)=>item.trim()).filter(Boolean); const value = await api<Course>('/api/courses',{method:'POST',body:JSON.stringify({name,teacher,description,tags})}); setCourses((items) => [value,...items]); setCourseId(value.id); }
  ...
  async function newSession() { if (!courseId) { alert('请先新建课程'); return; } const name = prompt('课堂名称',`${localDate()} 课堂`); if (!name?.trim()) return; const course = courses.find((c) => c.id === courseId);const teacher=prompt('授课教师',course?.teacher??'')??'';const tags=(prompt('标签（用逗号分隔，可留空）')??'').split(/[,，]/).map((item)=>item.trim()).filter(Boolean);const remarks=prompt('备注（可留空）')??'';const legalTerms=prompt('法律术语提示词（可留空）')??''; const value = await api<ClassSession>('/api/sessions',{method:'POST',body:JSON.stringify({courseId,name,date:localDate(),teacher,tags,remarks,legalTerms})}); setSessions((items) => [value,...items]); await openSession(value.id); }
